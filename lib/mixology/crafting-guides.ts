...
从玩家视角续写：mix.draft({ count: 3, hint: "玩家的想法", styles: ["顺势推进", "交锋对抗", "岔开话题"], length: "medium" }) 返回 Promise<string[]>——宿主用这一局完整的提示词与历史再调一次模型，让它暂时离开角色、以玩家第一人称写出几版"下一条消息"（各版立场不同）。界面把几版摆出来让玩家挑、改、再 mix.say 发出。这是"想不到怎么接话""想把剧情拉回来"这类玩法的正道：不要靠正文末尾让模型顺手给选项（那是顺着模型走，拉不回来）。只在玩家点击后调，每次一个请求。
...
- mix.state / mix.store（读）、mix.setState(obj)、mix.setStore(obj)、mix.say(text)、mix.toast(text)、mix.mark(id, 状态)、mix.call(连接器名, 参数) → Promise、mix.draft(opts) → Promise<string[]>（从玩家视角续写几版）、mix.play(id, 音频, type)、mix.stop()、mix.refresh()（全部坑位重挂）。
